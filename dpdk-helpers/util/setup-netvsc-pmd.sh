...
GET_SECONDARY=$(ip -br link show master "$PRIMARY")
assert_success
MANA_MAC=$( echo "$GET_SECONDARY" | awk '{ print $3 }')
assert_success
# $ ethtool -i enP30832p0s0 | grep bus-info
# > bus-info: 7870:00:00.0
# get MANA device bus info to pass to DPDK
SECONDARY_INFO=$(ethtool -i "$SECONDARY")
assert_success
BUS_INFO_RAW=$(echo "$SECONDARY_INFO" | grep bus-info)
assert_success
...
assert_success
DEV_UUID=$(basename "$PRIMARY_DEVICE")
assert_success
