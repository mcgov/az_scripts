...
# helpers
function AssertSuccess([string] $rgName){
    if (-not $?){
        if ($cleanupFailure){
            DeleteResources($rgName);
        }
...
function DeleteResources([string] $rgname){
    ...
    az group delete -g $rgname -f 'Microsoft.Compute/virtualMachines' -y;
}
...
az group create --location $region --resource-group $rgname; AssertSuccess($ResourceGroupName)
...
az network vnet create --resource-group $rgname --name $vnet; AssertSuccess($ResourceGroupName)
az network nsg create -n $nsg -g $rgname -l westus3; AssertSuccess($ResourceGroupName) 
...
az network route-table create -n $route_0 -g $rgname; AssertSuccess($ResourceGroupName)
az network route-table create -n $route_a -g $rgname; AssertSuccess($ResourceGroupName)
az network route-table create -n $route_b -g $rgname; AssertSuccess($ResourceGroupName)


# create the subnets 
write-host "Creating subnets..."
az network vnet subnet create --resource-group $rgname --vnet-name test-vnet -n $subnet_0 --address-prefix '10.0.0.0/24' --network-security-group $nsg --route-table $route_0 ; AssertSuccess($ResourceGroupName)
az network vnet subnet create --resource-group $rgname --vnet-name test-vnet -n $subnet_a --address-prefix '10.0.1.0/24' --network-security-group $nsg --route-table $route_a; AssertSuccess($ResourceGroupName)
az network vnet subnet create --resource-group $rgname --vnet-name test-vnet -n $subnet_b  --address-prefix '10.0.2.0/24' --network-security-group $nsg --route-table $route_b; AssertSuccess($ResourceGroupName)
...
foreach ($i in 0,1,2){
    ...
    az network nic create --private-ip-address $ip_address -n mgmt-nic-vm-$i -g $rgname --accelerated-networking 1 --subnet $subnet_0 --vnet-name $vnet; AssertSuccess($ResourceGroupName)
}
write-host "Creating client-side nics..."
foreach ($i in 0,1){
    $id = $i + 4; 
    $ip_address = $subnet_a_prefix + '.' + $id
    az network nic create --private-ip-address $ip_address -n snd-nic-vm-$i -g $rgname --accelerated-networking 1 --subnet $subnet_a --vnet-name $vnet; AssertSuccess($ResourceGroupName)
}
...
foreach ($i in 0,2){
    ...
    az network nic create --private-ip-address $ip_address -n rcv-nic-vm-$i -g $rgname --accelerated-networking 1 --subnet $subnet_b --vnet-name $vnet; AssertSuccess($ResourceGroupName)
}
write-host "Creating routing rules..."
# drop all traffic for mgmt subnet subnet_0
az network route-table route create -g $rgname --name $subnet_0_drop_0 --address-prefix $mgmt_first_hop --next-hop-type None --route-table-name $route_0 ; AssertSuccess($ResourceGroupName)
az network route-table route create -g $rgname --name $subnet_a_drop_0 --address-prefix $mgmt_first_hop --next-hop-type None --route-table-name $route_a; AssertSuccess($ResourceGroupName)
az network route-table route create -g $rgname --name $subnet_b_drop_0 --address-prefix $mgmt_first_hop --next-hop-type None --route-table-name $route_b; AssertSuccess($ResourceGroupName)

# fwd traffic from b to a to fwder on a
az network route-table route create -g $rgname --name $subnet_a_fwd --address-prefix $subnet_b_dest_a_ip_prefix --next-hop-type VirtualAppliance --route-table-name $route_a --next-hop-ip-address $subnet_a_fwd_ip; AssertSuccess($ResourceGroupName)

# fwd all traffic from a to b to fwder on b
az network route-table route create -g $rgname --name $subnet_a_fwd --address-prefix $subnet_b_dest_a_ip_prefix --next-hop-type VirtualAppliance --route-table-name $route_b --next-hop-ip-address $subnet_b_fwd_ip; AssertSuccess($ResourceGroupName)

# drop all other traffic from a to b
az network route-table route create -g $rgname --name $subnet_a_drop_b --address-prefix $b_first_hop --next-hop-type None --route-table-name $route_a ; AssertSuccess($ResourceGroupName)

# and all other traffic from b to a
az network route-table route create -g $rgname --name $subnet_b_drop_a --address-prefix $a_first_hop --next-hop-type None --route-table-name $route_b; AssertSuccess($ResourceGroupName)
...
$mgmtVm = az vm create -n $fwd_vm_name -g $rgname --size $vmSize --image $imageUrn --ssh-key-values "$SshPublicKey" --nics mgmt-nic-vm-0 snd-nic-vm-0 rcv-nic-vm-0; AssertSuccess($ResourceGroupName)
# sender gets a mgmt nic and a nic on subnet a
write-host "Creating client..."
$sndVm = az vm create -n $snd_vm_name -g $rgname --size $vmSize --image $imageUrn --ssh-key-values "$SshPublicKey" --nics mgmt-nic-vm-1 snd-nic-vm-1; AssertSuccess($ResourceGroupName)
# receiver gets a mgmt nic and a nic on subnet b
write-host "Creating server..."
$rcvVM = az vm create -n $rcv_vm_name -g $rgname --size $vmSize --image $imageUrn --ssh-key-values "$SshPublicKey" --nics mgmt-nic-vm-2 rcv-nic-vm-2; AssertSuccess($ResourceGroupName)
...
try {
    az vm run-command invoke --resource-group $rgname  -n $fwd_vm_name --command-id "RunShellScript" --script "sudo mkfs.ext4 /dev/sdb; mkdir /tmp/build; sudo mount /dev/sdb /tmp/build; sudo chmod +rw /tmp/build; " ; AssertSuccess($ResourceGroupName)
} catch {
    try {
        az vm run-command invoke --resource-group $rgname  -n $fwd_vm_name --command-id "RunShellScript" --script "sudo mkfs.ext4 /dev/sdc; mkdir /tmp/build; sudo mount /dev/sdc /tmp/build; sudo chmod +rw /tmp/build; " ; AssertSuccess($ResourceGroupName)
    } catch {
...
az vm run-command invoke --resource-group $rgname  -n $fwd_vm_name --command-id "RunShellScript" --script "DEBIAN_FRONTEND=noninteractive git clone $az_scripts_git $build_disk_dir/az_scripts" ; AssertSuccess($ResourceGroupName)

# mark scripts executable and run the setup
write-host "Running DPDK installation..."
az vm run-command invoke --resource-group $rgname  -n $fwd_vm_name --command-id "RunShellScript" --script "DEBIAN_FRONTEND=noninteractive cd $build_disk_dir/az_scripts/dpdk-helpers; chmod +x ./*.sh; ./dpdk-test-setup.ubuntu.sh" ; AssertSuccess($ResourceGroupName)

if (-not $TryRunTest){
    Write-Host "Skipping attempt to run the test. Resource creation was successful!"
    return 0;
}
# make the l3fwd rules files. NOTE: not sure this works as expected yet.
write-host "Creating l3fwd rules files..."
az vm run-command invoke --resource-group $rgname  -n $fwd_vm_name --command-id "RunShellScript" --script "DEBIAN_FRONTEND=noninteractive cd $build_disk_dir/az_scripts/dpdk-helpers/l3fwd; chmod +x ./*.sh; ./create_l3fwd_rules_files.sh $a_first_hop $b_first_hop" ; AssertSuccess($ResourceGroupName)

# start the forwarder
write-host "Running DPDK l3fwd (async)..."
az vm run-command invoke --no-wait --resource-group $rgname  -n $fwd_vm_name --command-id "RunShellScript" --script "DEBIAN_FRONTEND=noninteractive cd $build_disk_dir/az_scripts/dpdk-helpers/l3fwd; ./run-dpdk-l3fwd.sh $build_disk_dir/az_scripts/dpdk-helpers/dpdk/build/examples/dpdk-l3fwd";
# start the receiver
write-host "Starting server (async)..."
az vm run-command invoke --no-wait --resource-group $rgname  -n $rcv_vm_name  --command-id "RunShellScript" --script "DEBIAN_FRONTEND=noninteractive sudo timeout 1200 sockperf server --tcp -i $subnet_b_rcv_ip";
# start the sender
